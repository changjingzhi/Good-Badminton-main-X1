...
from badminton_analysis.media.mjpeg_streamer import MJPEGStreamer
...
def main():
    ...
    parser.add_argument('--use-qnn', action='store_true', default=False, help='使用 AidLite QNN 推理引擎 (高通 NPU)')
    parser.add_argument('--stream-port', default=11451, type=int, help='MJPEG HTTP 流媒体端口，默认 11451；设为 0 禁用')
    parser.add_argument('--stream-resolution', default=None, type=str, help='MJPEG 推流分辨率，如 1024x576；默认不缩放')
    parser.add_argument('--stream-quality', default=50, type=int, help='MJPEG JPEG 质量 1-100，默认 50 (越低延迟越小但画质越差)')
    ...
    # --- MJPEG HTTP streamer (headless display) ---
    streamer = None
    if args.stream_port > 0:
        stream_resize = None
        if args.stream_resolution:
            try:
                w, h = args.stream_resolution.lower().split('x')
                stream_resize = (int(w), int(h))
            except ValueError:
                raise ValueError(
                    f"Invalid --stream-resolution '{args.stream_resolution}'. "
                    "Expected format: WIDTHxHEIGHT, e.g. 1024x576")
