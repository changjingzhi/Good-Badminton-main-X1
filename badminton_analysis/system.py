...
def load_runtime_dependencies():
    ...
    try:
        ...
        from .detection.yolo_pose import YOLOPoseProcessor as _YOLOPoseProcessor, QNNPoseProcessor as _QNNPoseProcessor, Yolo11PoseQNNProcessor as _Yolo11PoseQNNProcessor
    ...
    QNNShuttlecockDetector = _QNNShuttlecockDetector
    Yolo11sBallDetector = _Yolo11sBallDetector
    ...
    QNNPoseProcessor = _QNNPoseProcessor
    Yolo11PoseQNNProcessor = _Yolo11PoseQNNProcessor
    vap = _vap
...
class BadmintonAnalysisSystem:
    def __init__(self, video_path, show_display=True, 
        ...
        if not _has_display:
            ...
            if self.show_display and self.mjpeg_streamer is None:
                print("No display detected – switching to headless mode (use --stream-port for HTTP streaming).")
                self.show_display = False
            elif self.show_display and self.mjpeg_streamer is not None:
                print("No X11 display – using MJPEG HTTP stream for display.")
        ...
        if self.use_qnn:
            print(f"使用 QNN 推理引擎 (高通 NPU)")
            self.rtmpose_processor = Yolo11PoseQNNProcessor(model_path=self.yolo_pose_model)
            self.yolo_ball_model = Yolo11sBallDetector(model_path=self.ball_model_path)
        else:
        ...
        self._court_check_interval = 3        # run matchTemplate every N frames
        self._court_check_counter = 0
        self._last_is_court = True            # assume court view initially
        self._match_scale = 0.5               # downscale factor for court matching

        self.frame_width = 0
        self.frame_height = 0
        self.performance_log_interval_frames = 150
    def process_video(self):
        """Process the input video.  When ``loop=True`` the video restarts
        from the beginning after reaching EOF, keeping the HTTP stream alive."""
        self.start_time = time.time()

        # ── One-time setup (shared across all loops) ──
        ...
        self.player_tracker = PlayerTracker(corners=corners, threshold=mid_height, history_size=30,
                                          detection_writer=None, fps=fps)  # writer set below
        ...
        while True:
            loop_count += 1
            if loop_count > 1 and not self._loop:
                break

            if loop_count > 1:
                print(f"\n--- 循环 #{loop_count} ---")
                # re-open the video from the beginning
                cap.release()
                cap = cv2.VideoCapture(self.video_path)
                if not cap.isOpened():
                    print("无法重新打开视频，退出循环。")
                ...
                self._write_metadata(fps, total_frames, video_duration,
                                     template_path, corners, roi_corners, mid_height)
                self.detection_writer = JsonlDetectionWriter(self.detections_path)
                self.player_tracker.detection_writer = self.detection_writer
            ...
            while cap.isOpened():
                ...
                do_detect = (frame_count % self.frame_skip == 1) or (self.frame_skip == 1)
                if do_detect:
                    frame, detect_frame_count = self._process_frame(
                        frame, template_gray, corners, roi_corners,
                        frame_count, out, detect_frame_count,
                    )
                else:
                    if frame is not None:
                        frame = self._draw_cached_overlays(frame, roi_corners)
                        self._output_frame(frame, out, frame_count)

            print(f"循环 #{loop_count} 完成 ({frame_count} 帧)")

        # ── Final wrap-up ──
        self.end_time = time.time()
        processing_time = self.end_time - self.start_time
        total_loops = loop_count - 1 if self._loop else 1

        print(f"\n处理完成 ({total_loops} 次循环):")
        print(f"原始视频时长: {video_duration:.2f} 秒")
        print(f"处理耗时: {processing_time:.2f} 秒")
        print(f"处理速度比: {processing_time / (video_duration * total_loops):.2f}x")
    ...
    def _process_frame(self, frame, template_gray, corners, roi_corners, frame_count, out, detect_frame_count):
        ...
        # ── Fast-path court matching (every N frames, downscaled) ──
        if self._court_check_counter % self._court_check_interval == 0:
            small_frame = cv2.resize(frame, None, fx=self._match_scale,
                                     fy=self._match_scale,
                                     interpolation=cv2.INTER_NEAREST)
            small_gray = cv2.cvtColor(small_frame, cv2.COLOR_BGR2GRAY)
            self._last_is_court = self.is_court_view(
                small_gray,
                cv2.resize(template_gray,
    ...
    # ------------------------------------------------------------------
    def _output_frame(self, frame, out, frame_no=0):
        """Write, display, and optionally save a processed frame.

        When *out* is ``None`` the frame is streamed / displayed but not
        written to disk (used for loop iterations after the first).
        """
        if frame is None:
            return
        ...
        if out is not None:
            out.write(frame)
        if self.save_images:
            name = str(frame_no) if frame_no else str(int(time.time() * 1000))
            cv2.imwrite(os.path.join(self.images_save_dir, f"{name}.png"), frame)

    # ------------------------------------------------------------------
    def _draw_cached_overlays(self, frame, roi_corners):
        """Draw all visual overlays using cached detection data (no AI inference).

        Called on skipped frames so that skeletons, trajectories, and stats
        remain painted on every frame of the output video.
        """
        # ── Pose ROI rectangle ──
        if self.show_pose_roi and roi_corners is not None:
            cv2.rectangle(frame, roi_corners[0], roi_corners[1], (255, 0, 0), 2)
            x1, y1 = roi_corners[0]
            cv2.putText(frame, "Pose ROI", (x1, max(24, y1 - 8)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2, cv2.LINE_AA)

        # ── Shuttlecock trajectory ──
        self.shuttlecock_tracker.handle_visualization(frame)

        # ── Player skeletons, dots, trajectories, stats panel ──
        self.player_pose_visualizer.draw_players(
            frame=frame,
            player_tracker=self.player_tracker,
            cached_movement_stats=self.cached_movement_stats,
        ...
        # ── Court minimap with player positions ──
        if self.show_court_trajectory:
            frame = self.court_trajectory_visualizer.draw_overlay(
                frame, self.player_tracker.court_history,
            )
    ...
    def _setup_court_annotation(self, template_color):
        """Set up court annotation – loads existing file or falls back
        to interactive annotation (requires a display)."""

        annotation_file = os.path.join(self.save_dir, 'court_annotations.txt')
